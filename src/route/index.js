...
class Product {
  ...
  constructor(
    ...
    this.id = ++Product.#count // Генеруємо унікальний id для товару (// this.id = Product.#list.length + 1 // Так не добре, бо якщо товар видалити - кількість зміниться)
    this.img = img
    this.title = title
...
class Purchase {
  ...
  static #bonusAccount = new Map() // словник - пошта + баланс бонусів по цій пошті
  ...
  static updateBonusBalance = (
    ...
    const amount = this.calcBonusAmount(price) // price * Purchase.#BONUS_FACTOR
  ...
  static updateById = (id, data) => {
    const purchase = Purchase.getById(id) // const purchase = Purchase.#list.find((item) => item.id === id)
...
router.post('/purchase-submit', function (req, res) {
  ...
  if (bonus || bonus > 0) {
    ...
    Purchase.updateBonusBalance(email, totalPrice, 0) // бонуси не списуємо
  }
  ...
  const purchase = Purchase.add(
    {
      totalPrice, // створення замовлення
      productPrice,
      deliveryPrice,
      amount,
      bonus,
